...
const TenantFault = ({ t }) => {
  ...
  return (
    <Fragment>
      <div>
        <Breadcrumb>
          <BreadcrumbItem>{t('Fault Detection & Diagnostics')}</BreadcrumbItem><BreadcrumbItem active>{t('Tenant Faults')}</BreadcrumbItem>
        </Breadcrumb>
      </div>
      ...
      <Card className="mb-3">
        <FalconCardHeader title="租户故障" light={false}>
          {isSelected ? (
...
export default withTranslation()(TenantFault);
