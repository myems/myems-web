...
const ShopfloorEnergyCategory = ({ setRedirect, setRedirectUrl,  t }) => {
  ...
  return (
    <Fragment>
      <div>
        <Breadcrumb>
          <BreadcrumbItem>{t('Shopfloor Data')}</BreadcrumbItem><BreadcrumbItem active>{t('Energy Category')}</BreadcrumbItem>
        </Breadcrumb>
      </div>
      ...
      <div className="card-deck">
        ...
        <CardSummary rate="0.0%" title={t('Reporting Period Input CATEGORY UNIT', { 'CATEGORY': '自来水', 'UNIT': '(M3)' })}
        ...
        <CardSummary rate="0.0%" title={t('Reporting Period Input CATEGORY UNIT', { 'CATEGORY': '天然气', 'UNIT': '(M3)' })}
